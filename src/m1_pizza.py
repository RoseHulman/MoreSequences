...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  100 Blue Dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 175)
    circle.fill_color = 'black'
    draw_points_on_circle(window, circle, 100, 'blue')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        center = rg.Point(points[k].x, points[k].y)
        outside = rg.Circle(center, 10)
        outside.fill_color = color
        outside.attach_to(window)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4: As suggested...'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 2
    circle.fill_color = 'yellow'
    pizza(window, circle, 300, 'black', 2)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    polygon(window, circle, 10, 'blue', 5)
...
def run_test_fancy_polygon():
    ...
    # Test 4:
    title = 'FANCY POLYGON test 4: As suggested'
    window = rg.RoseWindow(400, 400, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(number_of_lines):
        if k + hops_to_next_point < number_of_lines:
            ...
            line = rg.Line(points[k], points[n])
            n = n + 1
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
